...
本仓库是源代码版本。完整的本机模型和已安装依赖在桌面的 `PTE-Coach-Local-2026-09-16.zip` 中；从 GitHub 下载后，建议按 [分享与安装](docs/分享与安装.md) 安装 Python、FFmpeg、依赖并准备模型。两个运行模型已放在公开 Release，下载和校验方法见 [模型下载](models/模型下载.md)；仓库不使用 Git LFS。

安装 Python **3.12** 和 FFmpeg，并确保 `ffmpeg -version` 可运行。更完整的安装步骤见 [分享与安装](docs/分享与安装.md)。只做基础 CPU 启动时，在项目目录打开 PowerShell：
...
上面的命令适合先验证基础转写；要启用本地音素识别、单词练习和音标练习，请按 [分享与安装](docs/分享与安装.md) 下载 Release 模型并设置本地模型路径。
